...
static int bme680_chip_init(struct device *dev)
{
	...
	if (data->chip_id == BME680_CHIP_ID) {
		SYS_LOG_ERR("BME680 chip detected");
	} else {
		SYS_LOG_ERR("Bad BME680 chip id 0x%x", data->chip_id);
		return -ENOTSUP;
	}
...
static int bme680_init(struct device *dev)
{
	...
	data->i2c_slave_addr = CONFIG_BME680_I2C_ADDRESS;
...
DEVICE_AND_API_INIT(bme680, CONFIG_BME680_NAME, bme680_init, &bme680_data,
